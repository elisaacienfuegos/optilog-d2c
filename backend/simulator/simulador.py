...
def _imprimir(resultado: dict):
    e = resultado["envio"]
    print(f"\nEnvío: {e['peso_kg']} kg → {e['zona']}")
    print(f"  ELEGIDO: {resultado['elegido']}  (C={resultado['coste_total_C']})")
    print(f"  {'Transportista':<14}{'€':>7}{'días':>6}{'fiab':>7}{'C':>9}")
    for d in resultado["justificacion"]:
        marca = " ←" if d["transportista"] == resultado["elegido"] else ""
        print(f"  {d['transportista']:<14}{d['coste_eur']:>7}{d['plazo_dias']:>6}"
              f"{d['fiabilidad']:>7}{d['C']:>9}{marca}")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--peso", type=float, default=None)
    ap.add_argument("--zona", default=None, choices=["peninsula", "baleares", "canarias"])
    ap.add_argument("--w-coste", type=float, default=0.5)
    ap.add_argument("--w-plazo", type=float, default=0.3)
    ap.add_argument("--w-fiab", type=float, default=0.2)
    args = ap.parse_args()

    transportistas = catalogo_transportistas()
    pesos = Pesos(args.w_coste, args.w_plazo, args.w_fiab)

    if args.peso is not None and args.zona is not None:
        _imprimir(seleccionar_transportista(Envio(args.peso, args.zona), transportistas, pesos))
    else:
        # demo: varios escenarios representativos
        print("=== DEMO: pesos coste/plazo/fiabilidad =",
              f"{pesos.coste}/{pesos.plazo}/{pesos.fiabilidad} ===")
        for env in [Envio(0.5, "peninsula"), Envio(3.0, "peninsula"),
                    Envio(1.0, "canarias"), Envio(8.0, "canarias")]:
            _imprimir(seleccionar_transportista(env, transportistas, pesos))
